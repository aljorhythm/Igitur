    ...
    if ($type == 'json') {
        ...
        switch ($request) {
            case "get_all_symbols":
                echo json_encode(LogicalConnectiveSymbol::GET_SYMBOLS());
                break;
            case "phrases_add":
            ...
            case "category_add":
                ...
                break;
            case "phrases_from_category":
                $categoryId = filter_input($input, 'category_id', FILTER_SANITIZE_STRING);
                echo json_encode(LogicalConnectivePhrase::GET_PHRASES_FROM_CATEGORY($categoryId));
                break;
            ...
            case "category_all":
                ...
                break;
            case "category_from_symbol";
                $symbolId = filter_input($input, 'symbol_id', FILTER_SANITIZE_STRING);
                echo json_encode(LogicalConnectiveCategory::GET_CATEGORY_FROM_SYMBOL($symbolId));
                break;
            default: echo "error";
        }
    }
...
SQL;
    ...
    public static function GET_CATEGORY_FROM_SYMBOL($symbolId) {
        $db = (new DbController())->doConnect();
        $sql = <<<SQL
               SELECT 
    *
FROM
    Igitur.LogicalConnectiveCategory
where
    Igitur.LogicalConnectiveCategory.idLogicalConnectiveCategory = (SELECT 
            `LogicalConnectiveSymbol`.`LogicalConnectiveCategory_idLogicalConnectiveCategory`
        FROM
            `Igitur`.`LogicalConnectiveSymbol`
        WHERE
            Igitur.LogicalConnectiveSymbol.idLogicalConnectiveSymbol = $symbolId);
...
class LogicalConnectiveSymbol {
    ...
    public $symbol, $id, $category;

    public function __construct($id, $symbol, $category) {
        $this->id = $id;
        $this->symbol = $symbol;
        $this->category = $category;
    }

    public static function GET_SYMBOLS() {
        $db = (new DbController())->doConnect();
...
SQL;
    ...
    public static function GET_PHRASES_FROM_CATEGORY($categoryId) {
